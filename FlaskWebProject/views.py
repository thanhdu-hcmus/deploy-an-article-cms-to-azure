...
def new_post():
    ...
    return render_template('post.html', title='Create Post', imageSource=image_source_url, form=form)
...
def post(id):
    ...
    if form.validate_on_submit():
        ...
        return redirect(url_for('home'))
    return render_template('post.html', title='Edit Post', imageSource=image_source_url, form=form)
...
@app.route(Config.REDIRECT_PATH)  # Must match your app's redirect_uri set in AAD
def authorized():
